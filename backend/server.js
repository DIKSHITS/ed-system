...
const app = express();
const saltRounds = 10;
const port = process.env.PORT || 4000; // Use the environment variable or fallback to 4000

app.use(express.json());
app.use(cors({
  origin: 'https://localhost:4000', // Correct the URL format
  credentials: true
}));
...
app.delete('/api/delete-exam/:examId', async (req, res) => {
  try {
...
app.get('/api/doubts', async (req, res) => {
  try {
    const scheduleData = await Doubt.find();
    res.json(scheduleData);
  } catch (error) {
    console.error('Error fetching schedule data:', error);
    res.status(500).json({ error: 'Internal server error' });
...
app.put('/api/doubts/:id', async (req, res) => {
  try {
    ...
    console.error('Error updating doubt answer:', error);
    res.status(500).json({ message: "Internal server error" });
  }
});
...
app.post('/api/subscribe', async (req, res) => {
  try {
    ...
    const subscription = new Subscription({ courseId });
...
app.get('/api/subscriptions', async (req, res) => {
  try {
    const subscriptions = await Subscription.find().populate('courseId');
